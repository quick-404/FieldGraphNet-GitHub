...
positive rates near 20\%. Across five attack categories, the OOD structural branch achieves a
...
the structural branch contributes one operating point per category. The framework extends
analysis coverage beyond the attack classifier's protocol domain; its structural
branch achieves the highest mean F1 among the evaluated methods.
\end{abstract}
...
\begin{itemize}
  ...
  \item We evaluate the system on CIC IoT 2023 and CICIDS2017. The structural-branch comparison across
...
\textbf{Domain Router.} The router assigns training-domain membership from the
predicted protocol name, without consulting ports. When the identity is unavailable,
a port lookup later supplies a recognition fallback for the decision rule; it does
not change the router's domain assignment. Domain membership and protocol recognition
are distinct: a recognized protocol can still lie outside the attack classifier's
training domain.
...
one operating point per attack category for the OOD structural branch. Evaluation uses Python
...
categories. The structural branch achieved the highest reported mean F1, 0.5076, compared
...
\begin{table}[h]
	...
	\caption{F1 across five attack categories at a realized FPR near 20\%. Thresholds
		use benign scores. Baseline entries average five seeds; Ours is the OOD structural
		branch at one operating point per category.}
...
For Ours, the five OOD categories are predefined by capture; each score is the
minimum distance to a centroid fitted on in-domain structural fingerprints, not
the full routing and decision rule of Eq.~\ref{eq:fusion}.

Relative to the strongest baseline in each category, the structural branch improved F1 by
...
0.1636. The reported point estimates therefore favor the structural branch in three
...
structural-branch entry represents one operating point for the corresponding category.
...
Across five CIC IoT 2023 attack categories, the OOD structural branch achieved mean F1 of 0.5076,
...
To support reproducibility, we provide an anonymized artifact package containing the source code, experimental scripts, model checkpoints, and selected aggregate results. The raw CIC IoT 2023 and CICIDS2017 captures are not included and must be obtained from their respective sources. The package is publicly available at: \href{https://anonymous.4open.science/r/FieldGraphNet-GitHub-DE83/README.md}{\texttt{https://anonymous.4open.science/r/}\allowbreak\texttt{FieldGraphNet-GitHub-DE83/}\allowbreak\texttt{README.md}}.
